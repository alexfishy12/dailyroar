...
if(isset($_SESSION['account_type']) && $_SESSION['account_type']== "FA" ){
   ...
   if ($count > 0){
        echo "<div class='error'>This email address is already in the system. Please Go back.</div>";
    }
   ...
   if ($count == 0) {
        ...
        if (!$result) {
            echo "<div class='error'>An error retrieving the active semester has been encountered. Please Try again later.</div>";
            die();
        }
        $active_semester = $result->fetch_assoc()['ID'];

        $sql = "INSERT INTO Students VALUES (NULL, $active_semester, '$first_name', '$last_name', '$program', '$major1', ";
        if (empty($major2)) {
            $sql = $sql . "NULL, ";
        }
        else {
